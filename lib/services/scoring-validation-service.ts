...
export class ScoringValidationService {
  ...
  static async validateVersionForPublication(
    ...
    for (const node of version.nodes) {
      ...
      if (node.isMandatory && !node.applicabilityRules.some((r) => r.effectType === 'SHOW')) {
        warnings.push({
  ...
  static async validateEvaluationCompleteness(
    ...
    const answeredNodeIds = new Set(
      evaluation.answers.map((a) => a.nodeId)
    );
    const requiredNodes = evaluation.version.nodes.filter(
      (n) => n.isMandatory
    );
    ...
    switch (node.answerType) {
      case 'OPTION_SINGLE':
        if (!answer.valueString) {
          ...
          const valid = node.options.some((o) => o.value === answer.valueString);
          if (!valid) {
            errors.push({
              code: 'INVALID_OPTION',
