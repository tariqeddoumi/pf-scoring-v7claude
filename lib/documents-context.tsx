...
interface DocumentContextType {
  ...
  uploadVersion: (documentId: string, version: Omit<DocumentVersion, 'id'>) => void;
...
export function DocumentProvider({ children }: { children: React.ReactNode }) {
  ...
  const uploadVersion = (documentId: string, version: Omit<DocumentVersion, 'id'>) => {
    setDocuments(prev =>
      prev.map(doc => {
  ...
  const getDocumentsByProject = (projectId: string) => {
    return documents.filter(doc => doc.projectId === projectId && !doc.archived);
  };

  const getDocumentsByClient = (clientId: string) => {
    return documents.filter(doc => doc.clientId === clientId && !doc.archived);
  };

  const getDocumentsByEvaluation = (evaluationId: string) => {
    return documents.filter(doc => doc.evaluationId === evaluationId && !doc.archived);
  };
