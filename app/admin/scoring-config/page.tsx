...
const DEFAULT_CONFIG: ScoringConfig = {
  domains: [
    { id: 'D1', name: 'Sponsor & Actionnaires', weight: 10, minWeight: 5, maxWeight: 15, description: 'Qualité et stabilité du sponsor' },
    { id: 'D2', name: 'Caractéristiques Projet', weight: 10, minWeight: 5, maxWeight: 15, description: 'Profil technique et commercial' },
    { id: 'D3', name: 'Risque Construction', weight: 15, minWeight: 10, maxWeight: 20, description: 'EPC et gestion construction' },
    { id: 'D4', name: 'Risque Marché', weight: 10, minWeight: 5, maxWeight: 15, description: 'Demand et contrats' },
    { id: 'D5', name: 'Risque Opérationnel', weight: 10, minWeight: 5, maxWeight: 15, description: 'O&M et exploitation' },
    { id: 'D6', name: 'Risque Contrepartie', weight: 10, minWeight: 5, maxWeight: 15, description: 'Qualité acheteur/off-taker' },
    { id: 'D7', name: 'Structure Financière', weight: 15, minWeight: 10, maxWeight: 20, description: 'Financement et gestion' },
    { id: 'D8', name: 'Risque Juridique', weight: 10, minWeight: 5, maxWeight: 15, description: 'Documentation et contrats' },
    { id: 'D9', name: 'ESG & Climat', weight: 10, minWeight: 5, maxWeight: 15, description: 'Environnement, social, gouvernance' },
  ],
  noGoThresholds: {
    dscr: 1.1,
    equity: 20,
...
export default function ScoringConfigPage() {
  ...
  const handleWeightChange = (id: string, newWeight: number) => {
    setConfig(prev => ({
      ...
      domains: prev.domains.map(d =>
        d.id === id ? { ...d, weight: Math.max(d.minWeight, Math.min(d.maxWeight, newWeight)) } : d
      ),
    }));
    setSaved(false);
    setError('');
  ...
  const handleSave = () => {
    if (!isWeightValid) {
      setError(`La somme des poids doit égaler 100% (actuellement: ${totalWeight}%)`);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-2">Personnalisez les poids des domaines et les seuils NO-GO</p>
      </div>
            ...
            <div key={domain.id} className="bg-slate-700 rounded-lg p-4">
              <div className="mb-3">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <h3 className="font-semibold text-white">{domain.id}: {domain.name}</h3>
              ...
              <div className="flex items-center space-x-4">
                <input
                  ...
                  value={domain.weight}
                  onChange={(e) => handleWeightChange(domain.id, parseInt(e.target.value))}
                ...
                />
                <span className="text-xs text-slate-400">{domain.minWeight}% - {domain.maxWeight}%</span>
              </div>
            </div>
          ))}
        </div>
