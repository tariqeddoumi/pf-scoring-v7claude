  ...
  if (error || !user) {
    return (
      <div className="space-y-6">
        <Link href="/users" className="inline-flex items-center space-x-2 text-slate-400 hover:text-white">
          <ArrowLeft size={20} />
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">{user.nom} {user.prenom}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-xs font-semibold text-slate-400 uppercase">Rôle</p>
          </div>
          <p className="mt-2">
            <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${roleColors[user.role]}`}>
        ...
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4">
          ...
          <p className="mt-2 text-white">{new Date(user.createdAt).toLocaleDateString('fr-FR')}</p>
        </div>
      </div>

      {/* User Details Card */}
      <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <h2 className="text-lg font-semibold text-white mb-4">Informations complètes</h2>
        <div className="space-y-4">
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase">Nom complet</p>
            <p className="mt-1 text-white">{user.nom} {user.prenom}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase">Adresse email</p>
            <p className="mt-1 text-white">{user.email}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase">Rôle d'accès</p>
            <p className="mt-1">
              <span className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${roleColors[user.role]}`}>
                {roleLabels[user.role]}
              </span>
