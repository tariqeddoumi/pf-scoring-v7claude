...
export default function ExportsPage() {
  ...
  const handleExport = async (format: string) => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: error instanceof Error ? error.message : 'Erreur lors de l\'export' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-2">Exporter les données en différents formats</p>
      </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {EXPORT_OPTIONS.find(o => o.id === selectedExport)?.formats.map(format => (
                  <button
                    key={format}
            ...
            <div className="flex gap-3 pt-4">
              <Button
                ...
                disabled={!selectedFormat || (selectedExport === 'evaluation' && !selectedEvaluation) || loading}
      ...
      <Card className="border-slate-700 bg-slate-900">
        ...
        <CardContent className="space-y-2 text-sm text-slate-400">
          <p>• <strong>PDF</strong> : Format de lecture seule, idéal pour l'archivage et la distribution</p>
          <p>• <strong>Excel</strong> : Format éditable avec formules, parfait pour l'analyse supplémentaire</p>
          <p>• <strong>Word</strong> : Format documents structurés, idéal pour les rapports</p>
          <p>• <strong>CSV</strong> : Format texte universel, compatible avec tous les outils</p>
        </CardContent>
      </Card>
    </div>
  );
