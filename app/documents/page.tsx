...
import { FileText, Upload, Trash2, Archive, Download, Search, ChevronDown, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function DocumentsPage() {
  const { documents, deleteDocument, archiveDocument, uploadVersion } = useDocuments();
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-2">Gestion centralisée des documents de projets et clients</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-blue-400">{documents.filter(d => d.type === 'contract').length}</div>
            <div className="text-sm text-slate-400">Contrats</div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="pt-6">
            <div className="text-2xl font-bold text-purple-400">{documents.filter(d => d.type === 'audit').length}</div>
      ...
      <div className="space-y-3">
        {filteredDocs.length === 0 ? (
          ...
          filteredDocs.map(doc => (
            <Card key={doc.id} className="bg-slate-800 border-slate-700 overflow-hidden">
              <CardContent className="p-0">
                    ...
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2 py-1 rounded text-xs font-medium ${DOCUMENT_TYPE_COLORS[doc.type]}`}>
                ...
                {expandedDoc === doc.id && (
                  <div className="border-t border-slate-700 p-4 bg-slate-900/50 space-y-4">
                    {/* Versions */}
                    <div>
                      <h4 className="font-semibold text-slate-300 mb-3">Historique des Versions</h4>
                      <div className="space-y-2">
                        {doc.versions.map((version, idx) => (
                          <div key={version.id} className="p-3 bg-slate-800 rounded border border-slate-700 text-sm">
                            <div className="flex items-center justify-between">
                              <div>
                                <div className="font-medium text-slate-200">
                                  v{version.versionNumber}
                                  {idx === doc.versions.length - 1 && <span className="text-cyan-400 ml-2">(actuelle)</span>}
                                </div>
                                <div className="text-xs text-slate-500 mt-1">
                                  {version.fileName} • {formatFileSize(version.fileSize)}
                                </div>
                                {version.changeNote && (
                                  <div className="text-xs text-slate-400 mt-1">Note: {version.changeNote}</div>
                                )}
                                <div className="text-xs text-slate-600 mt-1">
                                  {new Date(version.uploadedAt).toLocaleString('fr-FR')} par {version.uploadedBy}
                                </div>
                              </div>
      ...
      <Card className="bg-slate-900 border-slate-700">
        <CardHeader>
          <CardTitle className="text-base">À propos de la Gestion des Documents</CardTitle>
        </CardHeader>
        <CardContent className="space-y-2 text-sm text-slate-400">
          <p>✓ <strong>Versioning</strong> : Suivez l'historique complet des documents avec notes de mise à jour</p>
          <p>✓ <strong>Archivage</strong> : Archivez les anciens documents pour garder le système propre</p>
          <p>✓ <strong>Audit Trail</strong> : Tous les uploads et modifications sont automatiquement enregistrés</p>
          <p>✓ <strong>Attribution</strong> : Chaque document est lié à un projet, client, ou évaluation</p>
          <p>✓ <strong>Recherche</strong> : Trouvez rapidement les documents par nom, type, ou contenu</p>
        </CardContent>
      </Card>
    </div>
  );
